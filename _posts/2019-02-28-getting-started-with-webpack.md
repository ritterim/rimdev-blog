...
Front end technologies can sometimes feel Kool aid-y. Even more so when super fans hit you with the `It's <insert_tech_here> or nothing` energy. This was how I felt about webpack until I watched the Webpack Fundamentals course on [Frontend Masters](https://frontendmasters.com/courses/webpack-fundamentals/). I cannot remommend the course enough if you are able to get your hands on it. Lets just go over stuff I think is worth knowing to become productive with webpack. 

## Should you use webpack?

It depends, but I personally think if you write highly structured javascript with your code organized into modules and/or across various files and directories, webpack can help by pulling them together into a script that you can include in your html. The benefit being that you don't have to manage adding script tags to your html file and figure out what order to add them in to make stuff work. 
...
- **Entry:** The is the starting point for webpack when determining out what goes into your bundle. It will figure out a dependency graph and pull imported items into the resulting bundle (Known as the output file).
...
___
Project summary:
- `tracker.js` is a utility that tracks usages for analytics
- `accordion.js` and `counter.js` respectively contain the logic for an accordion and a counter. They both import and make use of the tracker utility.
- `index.js` is our entry file and pulls in the accordion and counter files for usage on the webpage.
- Finally, the `dist` directory contains an `index.html` page which inludes a script link to a `main.js` file (which will be generated by webpack).
___
...
![Result](/images/counter_accordion.png "Web app")
...
![Result](/images/missing_loader_error.png "Loader error")
...
+ then create a `webpack.config.js` file to the root of your project with the following:
...
![Result](/images/counter_accordion_styled.png "Styled web app")

...
You _used_ to be able to do this using the [Astro.redirect](https://docs.astro.build/en/reference/errors/static-redirect-not-available/) method, but it has been deprecated as of version 2.6, unless you're using Server Side RFendering then you good. If using static site generation (like we do) you can use this approach that Astro recommends using the `meta refresh attribute`.
